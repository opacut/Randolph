...
public class Controller2D : RaycastController {
    ...
    public void Move(Vector2 moveAmount, Vector2 input, bool standingOnPlatform = false) {
        ...
        if (!Mathf.Approximately(moveAmount.x, 0)) {
            collisions.faceDir = (int) Mathf.Sign(moveAmount.x);
        }

        HorizontalCollisions(ref moveAmount);
        if (!Mathf.Approximately(moveAmount.y, 0)) {
    ...
    void VerticalCollisions(ref Vector2 moveAmount) {
        ...
        for (int i = 0; i < verticalRayCount; i++) {
            ...
            if (hit) {
                if (hit.collider.tag == "Through") {
                    if (Mathf.Approximately(directionY, 1) || Mathf.Approximately(hit.distance, 0)) {
                        continue;
                    }
                    if (collisions.fallingThroughPlatform) {
                        continue;
                    }
                    if (Mathf.Approximately(playerInput.y, -1)) {
        ...
        if (collisions.climbingSlope) {
            ...
            Vector2 rayOrigin = ((Mathf.Approximately(directionX, -1)) ? raycastOrigins.bottomLeft : raycastOrigins.bottomRight) + Vector2.up * moveAmount.y;
            RaycastHit2D hit = Physics2D.Raycast(rayOrigin, Vector2.right * directionX, rayLength, collisionMask);
    ...
    void DescendSlope(ref Vector2 moveAmount) {
        ...
        if (!collisions.slidingDownMaxSlope) {
            ...
            if (hit) {
                float slopeAngle = Vector2.Angle(hit.normal, Vector2.up);
                if (!Mathf.Approximately(slopeAngle, 0) && slopeAngle <= maxSlopeAngle) {
